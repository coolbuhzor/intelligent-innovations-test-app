    ...
    >
      ...
      <div className="w-full ">
        <div className="w-full flex justify-between mb-5 overflow-x-auto ">
          {headings.map((item, i) => (
            <div
              className={`header-item first:rounded-l-[10px] last:border-r-0 last:rounded-r-[10px] border-r bg-white w-full py-2 flex justify-around items-center text-[10px] md:text-base`}
              key={i}
            >
              {item.name}
              <SelectCaret className="hidden sm:block" />
            </div>
          ))}
        </div>
